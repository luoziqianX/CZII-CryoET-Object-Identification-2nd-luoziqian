...
from utils.training_data import (
    ...
    DEFAULT_TEST_TOMO_TYPES,
    DEFAULT_TRAIN_DATA_DIR,
    DEFAULT_TRAIN_NAMES,
    DEFAULT_TRAIN_TOMO_TYPES,
    DEFAULT_VALID_NAMES,
    build_validation_patch_data,
    load_labeled_files,
)
...
def label2dist(label, resolution=[1, 1, 1], dtype=None, num_classes=6):
    # label shape D, H, W 0-num_classes
    D, H, W = label.shape
    res = np.zeros((num_classes, D, H, W), dtype=dtype)
    for label_id in range(num_classes):
        posmask = label == label_id  # D, H, W
        negmask = ~posmask
        res[label_id] = (
...
tomo_type_list = DEFAULT_TRAIN_TOMO_TYPES
test_tomo_type_list = DEFAULT_TEST_TOMO_TYPES
data_type = DEFAULT_DATA_TYPES

train_files = load_labeled_files(
    TRAIN_DATA_DIR, train_names, tomo_type_list, data_type, progress_factory=tqdm
)
valid_files = load_labeled_files(
    TRAIN_DATA_DIR,
    valid_names,
    test_tomo_type_list,
    data_type,
...
# %%
val_patched_data = build_validation_patch_data(valid_files, val_patch_sizes)
